...
class AddPackagingProvider with ChangeNotifier{
  ...
  void setView(BuildContext context,{bool isUpdate=false,PackagingData data}){
  ...
  void addPackaging()async{
      ...
      Map<String,dynamic> response = await Api_Client.Request(url: isUpdate ? URL.VendorUpdatePackaging+packaging.id.toString() : URL.VendorAddPackaging,method: Method.POST,body: body);
      try {
        if (response.containsKey(AppConstant.status) && response[AppConstant.status]==true) {
          title.text='';
          subtitle.text='';
          price.text='';
          Navigator.of(context).pop(PackagingData.fromJson(response[AppConstant.data]));
