...
Widget DefaultTextField({@required TextEditingController controller,FocusNode focusNode,String label,Color backgroundColor,IconData prefixIcon,Widget suffixIcon,
  int maxLine=1,TextInputType textInputType=TextInputType.text,bool obscureText=false,String hint,bool enable=true,Function onTap,bool isRequired=false,bool borderEnable=true,bool enableValidation=true}){
  return GestureDetector(
    ...
    child: AbsorbPointer(
      ...
      child: TextFormField(
        ...
        validator: enableValidation ? (String value) {
          if (value.isEmpty) {
            if(focusNode!=null)
        ...
        decoration: InputDecoration(
            filled: true,
            contentPadding: EdgeInsets.only(left: 10,right: 10,top: label!=null ? Dimension.Padding : 5 ,bottom: label!=null ? Dimension.Padding : 5),
            fillColor: enable ? backgroundColor ?? Themes.Primary.withAlpha(15) : Colors.transparent,
            enabledBorder: borderEnable ? textFieldBorder() : InputBorder.none,
            focusedBorder: borderEnable ? textFieldBorder() : InputBorder.none,
            errorBorder: borderEnable ? textFieldBorder(isError: true) : InputBorder.none,
...
textFieldBorder({bool isError=false}){
  return OutlineInputBorder(
    borderSide: BorderSide(width: 2,color: isError ? Themes.Red : Themes.TexftFieldBorder),
  );
}